...
class UserSettingsAction__ extends YesWikiAction
{
    ...
    private function getUser(array $get): ?User
    {
        ...
        if ($this->wiki->UserIsAdmin() && (
            ...
            if (!empty($this->wantedUserName)) {
                ...
                $user = $this->userManager->getOneUserByName($this->wantedUserName);
                ...
                $user = $this->userManager->getOneUserByEmail($this->wantedEmail); // In this case we need to load the right user
            ...
            $user = isset($userFromSession['name']) ? $this->userManager->getOneUserByName($userFromSession['name']) : null;
    ...
    private function update(array $post, User $user)
    {
        if ($this->adminIsActing || $this->userLoggedIn) {
            try {
                ...
                $user = $this->userManager->getOneUserByEmail($sanitizedPost['email']);
