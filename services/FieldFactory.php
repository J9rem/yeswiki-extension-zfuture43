    ...
    private function checkCacheFolderExistence()
    {
        try {
            if (!file_exists(self::CACHE_PATH) || !is_dir(self::CACHE_PATH)) {
                throw new Exception("ERROR ! : Folder `".self::CACHE_PATH."/` not existing in the root folder on the website host ! Can you create it ? ");
            }

            if (!is_writable(self::CACHE_PATH)) {
                throw new Exception("ERROR ! : Folder `".self::CACHE_PATH."cache/` is not writable ! Can you give it write acces by ftp for example (code 770) ?");
            }
        } catch (Exception $th) {
