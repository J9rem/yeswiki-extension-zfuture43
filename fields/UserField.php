...
class UserField extends BazarField
{
    ...
    protected function renderInput($entry)
    {
        ...
        if (!empty($loggedUser)) {
            $associatedUser = $userManager->getOneUserByName($loggedUser['name']);
            if (!empty($associatedUser['name'])) {
                ...
                if ($value !== $loggedUser['name'] && $this->getWiki()->UserIsAdmin()) {
                    $associatedUser = $userManager->getOneUserByName($value);
                }
    ...
    public function formatValuesBeforeSave($entry)
    {
        ...
        if ($this->getWiki()->UserIsAdmin()
            ...
            $userManager = $this->getService(UserManager::class);
            $existingUser = $userManager->getOneUserByEmail($entry[$this->emailField]);
    ...
    {
        $userManager = $this->getService(UserManager::class);
        return !empty($userManager->getOneUserByName($userName));
    }

    private function isUserByEmail(string $email): bool
    {
        $userManager = $this->getService(UserManager::class);
        return !empty($userManager->getOneUserByEmail($email));
    }
    ...
    private function updateEmailIfNeeded(string $userName, string $email)
    {
        if ($this->getAutoUpdateMail() && !empty($userName) && !empty($email)) {
            ...
            $userController = $this->getService(UserController::class);
            $userManager = $this->getService(UserManager::class);
            $user = $userManager->getOneUserByName($userName);
            $loggedUser = $authController->getLoggedUser();
            if (!empty($user)
                    && (
